...
# Note that some of the teams in the data set were not actually part of the conference for the entirety of the BCS
...
# Rutgers, and Louisville were not added until 2014 which is beyond the BCS era.
...
# Get all P5 school names (remember to omit Maryland and Rutgers for the BCS era, keep for Playoff era):
...
df.test <- df.bcs %>% full_join(df.ap, by = "year")
df.test <- df.test %>% rowwise() %>% mutate(top15 = ifelse(rank < 16 & school.x == school.y, 
                                                           1, 0))
df.test <- df.test %>% select(year, school.x, winpct, wontitle, top15)
df.test <- df.test %>% filter(top15==1)
colnames(df.test)[2] <- "school"
